function main_file(fname)
    ...
    video_right = VideoReader(fullfile('./', fname_right)); % Reading the Right Eye Video
    video_left = VideoReader(fullfile('./', fname_left)); % Reading the Left Eye Video
    ...
    [hour_right var1 min_right var2 sec_right] = textread(fullfile('./', fname_right_txt), '%d %c %d %c %f'); % Getting the Hour,Minutes,Seconds separately for the Right Eye
    time_rgt = [hour_right,min_right,sec_right];
    
    [hour_left var1 min_left var2 sec_left] = textread(fullfile('./', fname_left_txt), '%d %c %d %c %f'); % Getting the Hour,Minutes,Seconds separately for the Left Eye
    ...
    dlmwrite('./Area_SOL_Time_CSV/RawAreas_Right.csv',area_right,'-append');
    dlmwrite('./Area_SOL_Time_CSV/RawAreas_Left.csv',area_left,'-append');
    
    dlmwrite('./Area_SOL_Time_CSV/Size_Right.csv',size(area_right,2),'-append');
    dlmwrite('./Area_SOL_Time_CSV/Size_Left.csv',size(area_left,2),'-append');
...
%     all_areas(i,:) = [area_right area_left];
    ...
    sol = [state_of_light_right' state_of_light_right'];
    
    dlmwrite('./Area_SOL_Time_CSV/SOL_Right.csv',sol_right','-append');
    dlmwrite('./Area_SOL_Time_CSV/SOL_Left.csv',sol_left','-append');
...
% Will need to change this part based on the Retrain/Test/Discard 
% If Discard then no problem as the processing iteslf does not start
% If Retrain then training_size will increase by 1
% If Test then directly use the stored all_theta and predict the value.
